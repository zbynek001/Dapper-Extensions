...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        string Update(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters, string keyName = null, IEnumerable<string> properties = null);
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string Select(IClassMapper classMap, IPredicate predicate, IList<ISort> sort, IDictionary<string, object> parameters, string hints = null)
        {
            ...
            {
                sql.Append(" ORDER BY ")
                    .Append(sort.Select(s => GetColumnName(classMap, s.PropertyName, false/*, false*/) + (s.Ascending ? " ASC" : " DESC")).AppendStrings());
            }
        ...
        public virtual string SelectPaged(IClassMapper classMap, IPredicate predicate, IList<ISort> sort, int page, int resultsPerPage, IDictionary<string, object> parameters)
        {
            ...
            string orderBy = sort.Select(s => GetColumnName(classMap, s.PropertyName, false/*, false*/) + (s.Ascending ? " ASC" : " DESC")).AppendStrings();
        ...
        {
            var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.IsAnyKeyType(KeyType.Identity)));
            if (!columns.Any())
            {
                throw new ArgumentException("No columns were mapped.");
            }

            var columnNames = columns.Select(p => GetColumnName(classMap, p, false/*, false*/));
        ...
        public virtual string Update(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters, string keyName = null, IEnumerable<string> properties = null)
        {
            if (predicate == null)
            {
            ...
            var setSql =
                columns.Select(
                    p =>
                    string.Format(
                        "{0} = {1}{2}", GetColumnName(classMap, p, false/*, false*/), Configuration.Dialect.ParameterPrefix, p.Name));
        ...
        public virtual string GetColumnName(IClassMapper map, IPropertyMap property, bool includeAlias/*, bool includeTablePrefix = true*/)
        {
            ...
            return Configuration.Dialect.GetColumnName(null /*includeTablePrefix ? GetTableName(map) : null*/, property.ColumnName, alias);
        }

        public virtual string GetColumnName(IClassMapper map, string propertyName, bool includeAlias/*, bool includeTablePrefix = true*/)
        {
            ...
            return GetColumnName(map, propertyMap, includeAlias/*, includeTablePrefix*/);
        }
        ...
        public virtual string BuildSelectColumns(IClassMapper classMap)
        {
            var columns = classMap.Properties
                .Where(p => !p.Ignored)
                .Select(p => GetColumnName(classMap, p, true/*, false*/));
